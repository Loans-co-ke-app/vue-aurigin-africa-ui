...
import { AuriginAfricaRouteMetaType, UserStateRoleType } from "@/types";
...
router.beforeEach((to, from, next) => {
  ...
  if (store.isAuthenticated) {
    ...
    // if (!store.isUserActive && to.name !== "activateAccount") {
    ...
    if (meta.requiresAuth && meta.roles) {
      if (meta.roles.includes(store.authStateUser!.role as UserStateRoleType)) {
        return next();
      } else {
